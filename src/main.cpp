 ...
 * We are ~100 times smaller then aria now (2015-06-23), set minRelayTxFee only 10 times higher
...
{
    //Check to see if the zAriA is properly signed
...
int64_t GetBlockValue(int nHeight)
{
    if (Params().NetworkID() == CBaseChainParams::TESTNET) {
      ...
      int64_t nSubsidy = 0;
      if (nHeight == 0) {
          nSubsidy = 450000000 * COIN; //Central Nodes Servers + Pre-Sale
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZAriAStake)
{
    int64_t ret = 0;
	...
	// 90% for Masternodes from block 1000
     if (nHeight <= 100) {
		  ...
		  ret = blockValue  / 100 * 90;               // %90
                  ...
                  //When zAriA is staked, masternode only gets 2 AriA
                  ret = 3 * COIN;
                 if (isZAriAStake)
                  ret = 2 * COIN;
     }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * addresses should still be handled by the typical aria based undo code
         * */
...
bool UpdateZAriASupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
    BlockToZerocoinMintList(block, listMints);
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
   ...
   //Track zAriA money supply in the block index
    if (!UpdateZAriASupply(block, pindex))
...
bool ContextualCheckZerocoinStake(CStakeInput* stake)
{
    if (chainActive.Height() + 1 < Params().Zerocoin_Block_V2_Start())
     ...
     if (CZAriAStake* zAriA = dynamic_cast<CZAriAStake*>(stake)) {
        CBlockIndex* pindex = zAriA->GetIndexFrom();
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zAriA->GetValue()));
        if (nChecksum200 != zAriA->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zAriA->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        CStakeInput* stake = nullptr;
         ...
         if (stake->IsZAriA() && !ContextualCheckZerocoinStake(stake))
...
{
    {
        ...
        if (!state.fSyncStarted && !pto->fClient && fFetch /*&& !fImporting*/ && !fReindex) {
            ...
            if (nSyncStarted == 0 || pindexBestHeader->GetBlockTime() > GetAdjustedTime() - 6 * 60 * 60) { // NOTE: was "close to today" and 24h in AriA
