...
int GetBudgetPaymentCycleBlocks()
{
    ...
    if (Params().NetworkID() == CBaseChainParams::MAIN) return 40088;
...
CAmount CBudgetManager::GetTotalBudget(int nHeight)
{
    ...
    //get block value and calculate from that
      ...
      if (nHeight <= 100500 && nHeight > 100000) {
  	          nSubsidy = 10.5 * COIN;
      } else if (nHeight <= 101500 && nHeight > 100500) {
          nSubsidy = 0.5 * COIN; //light fork speed
      } else if (nHeight <= 102500 && nHeight > 101500) {
          nSubsidy = 27.5 * COIN;
      } else if (nHeight <= 110000 && nHeight > 102500) {
          nSubsidy = 75.5 * COIN;
      } else if (nHeight <= 150000 && nHeight > 110000) {
          nSubsidy = 0.007 * COIN; //lift transaction transfer
    	} else if (nHeight <= 230000 && nHeight > 110000) {
            nSubsidy = 250.5 * COIN;
