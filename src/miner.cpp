...
bool fGenerateAriAs = false;

// ***TODO*** that part changed in aria, we are using a mix with old one here for now

void AriAMiner(CWallet* pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateAriAs || fProofOfStake) {
        if (fProofOfStake) {
            ...
            while (IsInitialBlockDownload() || pwallet->IsLocked() || !fMintableCoins || nReserveBalance >= pwallet->GetBalance() || !masternodeSync.IsSynced()) {
                ...
                MilliSleep(5000);
                if (!fGenerateAriAs && !fProofOfStake)
                    continue;
            }
        ...
        if (fProofOfStake) {
            ...
            //Sign block with the zPIV key
                if (!SignBlockWithKey(*pblock, key)) {
                    LogPrintf("AriAMiner(): Signing new block with zPIV key failed \n");
                    continue;
                }
            } else if (!SignBlock(*pblock, *pwallet)) {
                LogPrintf("AriAMiner(): Signing new block with UTXO key failed \n");
                continue;
            }
...
void static ThreadAriAMiner(void* parg)
{
    boost::this_thread::interruption_point();
    CWallet* pwallet = (CWallet*)parg;
    try {
        AriAMiner(pwallet, false);
        boost::this_thread::interruption_point();
    } catch (std::exception& e) {
        LogPrintf("ThreadAriAMiner() exception");
    } catch (...) {
        LogPrintf("ThreadAriAMiner() exception");
    }

    LogPrintf("ThreadAriAMiner exiting\n");
}

void GenerateAriAs(bool fGenerate, CWallet* pwallet, int nThreads)
{
    static boost::thread_group* minerThreads = NULL;
    fGenerateAriAs = fGenerate;
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ThreadAriAMiner, pwallet));
}
