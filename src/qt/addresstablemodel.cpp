...
bool AddressTableModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        CTxDestination curAddress = CAriAAddress(rec->address.toStdString()).Get();
        if (index.column() == Label) {
            ...
            CTxDestination newAddress = CAriAAddress(value.toString().toStdString()).Get();
...
{
    // Update stealth address book model from AriA core
...
QString AddressTableModel::addRow(const QString& type, const QString& label, const QString& address)
{
    ...
    if (type == Send) {
        ...
        {
            LOCK(wallet->cs_wallet);
            if (wallet->mapAddressBook.count(CAriAAddress(strAddress).Get())) {
                editStatus = DUPLICATE_ADDRESS;
                return QString();
            }
        }
        ...
        strAddress = CAriAAddress(newKey.GetID()).ToString();
    } else {
        return QString();
    }
    ...
    {
        LOCK(wallet->cs_wallet);
        wallet->SetAddressBook(CAriAAddress(strAddress).Get(), strLabel,
...
bool AddressTableModel::removeRows(int row, int count, const QModelIndex& parent)
{
    ...
    {
        LOCK(wallet->cs_wallet);
        wallet->DelAddressBook(CAriAAddress(rec->address.toStdString()).Get());
    }
    return true;
}
...
QString AddressTableModel::labelForAddress(const QString& address) const
{
    {
        LOCK(wallet->cs_wallet);
        CAriAAddress address_parsed(address.toStdString());
