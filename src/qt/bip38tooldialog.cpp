...
void Bip38ToolDialog::on_encryptKeyButton_ENC_clicked()
{
    ...
    CAriAAddress addr(ui->addressIn_ENC->text().toStdString());
    if (!addr.IsValid()) {
        ui->statusLabel_ENC->setStyleSheet("QLabel { color: red; }");
...
void Bip38ToolDialog::on_decryptKeyButton_DEC_clicked()
{
    ...
    CAriAAddress address(pubKey.GetID());

    ui->decryptedKeyOut_DEC->setText(QString::fromStdString(CAriASecret(key).ToString()));
...
void Bip38ToolDialog::on_importAddressButton_DEC_clicked()
{
    ...
    CAriAAddress address(ui->addressOut_DEC->text().toStdString());
    CPubKey pubkey = key.GetPubKey();

    if (!address.IsValid() || !key.IsValid() || CAriAAddress(pubkey.GetID()).ToString() != address.ToString()) {
        ui->statusLabel_DEC->setStyleSheet("QLabel { color: red; }");
