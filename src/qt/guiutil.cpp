...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setFont(AriAAddressFont());
#if QT_VERSION >= 0x040700
    // We don't want translators to use own addresses in translations
    // and this is the only place, where this address is supplied.
    widget->setPlaceholderText(QObject::tr("Enter a rTw address (e.g. %1)").arg("b7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
    widget->setValidator(new AriAAddressEntryValidator(parent));
    widget->setCheckValidator(new AriAAddressCheckValidator(parent));
}
...
bool parseAriAURI(const QUrl& uri, SendCoinsRecipient* out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++) {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!AriAUnits::parse(AriAUnits::ARA, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseAriAURI(QString uri, SendCoinsRecipient* out)
{
    ...
    return parseAriAURI(uriInstance, out);
}

QString formatAriAURI(const SendCoinsRecipient& info)
{
    QString ret = QString(URI_SCHEME ":%1").arg(info.address);
    int paramCount = 0;

    if (info.amount) {
        ret += QString("?amount=%1").arg(AriAUnits::format(AriAUnits::ARA, info.amount, false, AriAUnits::separatorNever));
        paramCount++;
    }
...
{
    CTxDestination dest = CAriAAddress(address.toStdString()).Get();
...
bool GetStartOnSystemStartup()
{
    CFURLRef AriAAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    LSSharedFileListRef loginItems = LSSharedFileListCreate(NULL, kLSSharedFileListSessionLoginItems, NULL);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, AriAAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        ...
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, NULL, NULL, AriAAppUrl, NULL, NULL);
