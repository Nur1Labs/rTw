    ...
    if (role == Qt::DisplayRole || role == Qt::EditRole) {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return AriAUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, AriAUnits::separatorNever);
            else
                return AriAUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
...
QString RecentRequestsTableModel::getAmountTitle()
{
    ...
    if (this->walletModel->getOptionsModel() != NULL) {
        amountTitle += " (" + AriAUnits::name(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")";
    }
    return amountTitle;
}
