                                                                    ...
                                                                    model(0)
{
    ...
    ui->signatureOut_SM->setFont(GUIUtil::AriAAddressFont());
    ui->signatureIn_VM->setFont(GUIUtil::AriAAddressFont());
}
...
void SignVerifyMessageDialog::on_signMessageButton_SM_clicked()
{
    ...
    CAriAAddress addr(ui->addressIn_SM->text().toStdString());
    if (!addr.IsValid()) {
...
{
    CAriAAddress addr(ui->addressIn_VM->text().toStdString());
    if (!addr.IsValid()) {
    ...
    if (!(CAriAAddress(pubkey.GetID()) == addr)) {
        ui->statusLabel_VM->setStyleSheet("QLabel { color: red; }");
