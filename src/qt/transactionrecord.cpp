    ...
    if (wtx.IsCoinStake()) {
        ...
        if (!wtx.IsZerocoinSpend() && !IsMine(*wallet, address)) {
            ...
            for (unsigned int i = 1; i < wtx.vout.size(); i++) {
                ...
                if (ExtractDestination(wtx.vout[i].scriptPubKey, outAddress)) {
                    if (IsMine(*wallet, outAddress)) {
                        ...
                        sub.address = CAriAAddress(outAddress).ToString();
            ...
            if (wtx.IsZerocoinSpend()) {
                ...
                sub.type = TransactionRecord::StakeZAriA;
		...
		sub.debit -= wtx.vin[0].nSequence * COIN;
                ...
                sub.address = CAriAAddress(address).ToString();
                sub.credit = nNet;
            }
        }
        parts.append(sub);
        ...
        for (const CTxOut txout : wtx.vout) {
            ...
            if (ExtractDestination(txout.scriptPubKey, address))
                strAddress = CAriAAddress(address).ToString();
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || TransactionRecord::StakeZAriA || type == TransactionRecord::MNReward) {
