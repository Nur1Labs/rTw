...
void TransactionView::changedAmount(const QString& amount)
{
    ...
    if (model) {
        // Replace "," by "." so AriAUnits::parse will not fail for users entering "," as decimal separator
        QString newAmount = amount;
        newAmount.replace(QString(","), QString("."));

        if (AriAUnits::parse(model->getOptionsModel()->getDisplayUnit(), newAmount, &amount_parsed)) {
...
void TransactionView::exportClicked()
{
    ...
    if (model) {
        ...
        writer.addColumn(AriAUnits::getAmountColumnTitle(model->getOptionsModel()->getDisplayUnit()), 0, TransactionTableModel::FormattedAmountRole);
...
void TransactionView::computeSum()
{
    ...
    QString strAmount(AriAUnits::formatWithUnit(nDisplayUnit, amount, true, AriAUnits::separatorAlways));
