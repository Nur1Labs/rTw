// Copyright (c) 2010-2018 The Dash developers
...
{
    CAriAAddress addressParsed(address.toStdString());
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            CScript scriptPubKey = GetScriptForDestination(CAriAAddress(rcp.address.toStdString()).Get());
...
{
    QString strAddress = QString::fromStdString(CAriAAddress(address).ToString());
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        if (!out.fSpendable || !ExtractDestination(cout.tx->vout[cout.i].scriptPubKey, address))
            continue;
        mapCoins[QString::fromStdString(CAriAAddress(address).ToString())].push_back(out);
    }
}
...
{
    CTxDestination dest = CAriAAddress(sAddress).Get();
...
bool WalletModel::isMine(CAriAAddress address)
{
