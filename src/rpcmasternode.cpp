...
UniValue getpoolinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"current\": \"addr\",    (string) rTw address of current masternode\n"
...
UniValue listmasternodes(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || (params.size() > 1))
        throw runtime_error(
            ...
            "    \"addr\": \"addr\",      (string) Masternode rTw address\n"
    ...
    BOOST_FOREACH (PAIRTYPE(int, CMasternode) & s, vMasternodeRanks) {
        ...
        if (mn != NULL) {
            if (strFilter != "" && strTxHash.find(strFilter) == string::npos &&
                ...
                CAriAAddress(mn->pubKeyCollateralAddress.GetID()).ToString().find(strFilter) == string::npos) continue;
            ...
            obj.push_back(Pair("addr", CAriAAddress(mn->pubKeyCollateralAddress.GetID()).ToString()));
...
UniValue masternodecurrent (const UniValue& params, bool fHelp)
{
    ...
    if (winner) {
        ...
        obj.push_back(Pair("pubkey", CAriAAddress(winner->pubKeyCollateralAddress.GetID()).ToString()));
...
UniValue createmasternodekey (const UniValue& params, bool fHelp)
{
    ...
    return CAriASecret(secret).ToString();
}
...
UniValue getmasternodestatus (const UniValue& params, bool fHelp)
{
    if (fHelp || (params.size() != 0))
        throw runtime_error(
            ...
            "  \"addr\": \"xxxx\",        (string) rTw address for masternode payments\n"
            "  \"status\": \"xxxx\",      (string) Masternode status\n"
    ...
    if (pmn) {
        ...
        mnObj.push_back(Pair("addr", CAriAAddress(pmn->pubKeyCollateralAddress.GetID()).ToString()));
...
UniValue getmasternodewinners (const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "    \"winner\": {\n"
            "      \"address\": \"xxxx\",    (string) rTw MN Address\n"
...
UniValue decodemasternodebroadcast(const UniValue& params, bool fHelp)
{
	...
	resultObj.push_back(Pair("pubkeycollateral", CAriAAddress(mnb.pubKeyCollateralAddress.GetID()).ToString()));
	resultObj.push_back(Pair("pubkeymasternode", CAriAAddress(mnb.pubKeyMasternode.GetID()).ToString()));
