...
UniValue setgenerate(const UniValue& params, bool fHelp)
{
    ...
    } else // Not -regtest: start generate thread, return immediately
    {
        ...
        GenerateAriAs(fGenerate, pwalletMain, nGenProcLimit);
    }
...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "See https://en.AriA.it/wiki/BIP_0022 for full specification.\n"
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "rTw is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "rTw is downloading blocks...");
