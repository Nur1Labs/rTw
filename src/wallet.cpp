 ...
 * We are ~100 times smaller then aria now (2015-06-23), set minTxFee 10 times higher
 * so it's still 10 times lower comparing to aria.
...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    if (redeemScript.size() > MAX_SCRIPT_ELEMENT_SIZE) {
        std::string strAddr = CAriAAddress(CScriptID(redeemScript)).ToString();
...
bool CWallet::GetVinAndKeysFromOutput(COutput out, CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet)
{
    ...
    CAriAAddress address2(address1);
...
map<CAriAAddress, vector<COutput> > CWallet::AvailableCoinsByAddress(bool fConfirmed, CAmount maxCoinValue)
{
    vector<COutput> vCoins;
    AvailableCoins(vCoins, fConfirmed);

    map<CAriAAddress, vector<COutput> > mapCoins;
    BOOST_FOREACH (COutput out, vCoins) {
        ...
        mapCoins[CAriAAddress(address)].push_back(out);
    }
...
bool CWallet::SelectStakeCoins(std::list<CStakeInput*>& listInputs, CAmount nTargetAmount) const
{
	...
	CAriAStake* input = new CAriAStake();
        ...
        list<CZerocoinMint> listMints = walletdb.ListMintedCoins(true, true, true);
         for (CZerocoinMint mint : listMints) {
            ...
            if (mint.GetHeight() < chainActive.Height() - Params().Zerocoin_RequiredStakeDepth()) {
                CZAriAStake *input = new CZAriAStake(mint);
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CMutableTransaction& txNew, unsigned int& nTxNewTime)
{
    ...
    for (CStakeInput* stakeInput : listInputs) {
	    ...
	    CAmount nMinFee = 0;
            ...
            //Masternode payment
            FillBlockPayee(txNew, nMinFee, true, stakeInput->IsZAriA());
            ...
            //Mark mints as spent
            if (stakeInput->IsZAriA()) {
                CZAriAStake* z = (CZAriAStake*)stakeInput;
                if (!z->MarkSpent(this))
...
bool CWallet::SetAddressBook(const CTxDestination& address, const string& strName, const string& strPurpose)
{
    ...
    if (!fFileBacked)
        return false;
    if (!strPurpose.empty() && !CWalletDB(strWalletFile).WritePurpose(CAriAAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(strWalletFile).WriteName(CAriAAddress(address).ToString(), strName);
}
...
{
    {
        ...
        if (fFileBacked) {
            ...
            std::string strAddress = CAriAAddress(address).ToString();
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).ErasePurpose(CAriAAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CAriAAddress(address).ToString());
}
...
bool CWallet::AddDestData(const CTxDestination& dest, const std::string& key, const std::string& value)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).WriteDestData(CAriAAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination& dest, const std::string& key)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).EraseDestData(CAriAAddress(dest).ToString(), key);
}
...
void CWallet::AutoCombineDust()
{
    ...
    map<CAriAAddress, vector<COutput> > mapCoinsByAddress = AvailableCoinsByAddress(true, nAutoCombineThreshold * COIN);

    //coins are sectioned by address. This combination code only wants to combine inputs that belong to the same address
    for (map<CAriAAddress, vector<COutput> >::iterator it = mapCoinsByAddress.begin(); it != mapCoinsByAddress.end(); it++) {
    ...
    for (const COutput& out : vCoins) {
        ...
        if (vDisabledAddresses.size() > 0) {
            for (unsigned int i = 0; i < vDisabledAddresses.size(); i++) {
                if (vDisabledAddresses[i] == CAriAAddress(destMyAddress).ToString()) {
        ...
        for (unsigned int i = 0; i < vMultiSend.size(); i++) {
            ...
            CAriAAddress strAddSend(vMultiSend[i].first);
...
bool CWallet::CreateZAriAOutPut(libzerocoin::CoinDenomination denomination, libzerocoin::PrivateCoin& coin, CTxOut& outMint)
{
...
bool CWallet::CreateZerocoinMintTransaction(const CAmount nValue, CMutableTransaction& txNew, vector<CZerocoinMint>& vMints, CReserveKey* reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, const bool isZCSpendChange)
{
    ...
    while (true) {
        ...
        if (!CreateZAriAOutPut(denomination, coin, outMint)) {
...
bool CWallet::CreateZerocoinSpendTransaction(CAmount nValue, int nSecurityLevel, CWalletTx& wtxNew, CReserveKey& reserveKey, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vSelectedMints, vector<CZerocoinMint>& vNewMints, bool fMintChange,  bool fMinimizeChange, CAriAAddress* address)
{
...
bool CWallet::SpendZerocoin(CAmount nAmount, int nSecurityLevel, CWalletTx& wtxNew, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vMintsSelected, bool fMintChange, bool fMinimizeChange, CAriAAddress* addressTo)
{
